...
## MAJOR UPDATE: MW2 reaches real Present with zero crashes anywhere in the D3D9 pipeline — the remaining gap is pure CPU-emulation throughput, which is exactly your architectural strength

Eleven more real fixes landed since the last update (§64-77 in full — desktop-window threading, D3D9 caps/format/query gaps, display-mode mismatch, buffer sizing, a legacy-DirectDraw-probe crash inside real `dxcore.dll` resolved by intercepting one specific call, and a project-wide FPU/SSE control-state seeding bug). MW2 now creates a real D3D9 device, creates real resources/shaders/vertex declarations, and completes `IDirect3DDevice9::Present` calls with **zero crashes anywhere in device creation, resource creation, shader creation, or presentation**.

**The honest remaining gap, thoroughly profiled (§78-82):** MW2 hasn't rendered a frame yet because (a) its fullscreen-flip present path is a `d3d9.dll`-internal no-op on this backend (a scanout-surface flag sogen's surfaces don't set — not urgent to chase, since (b) is the real gate) and (b) MW2 hasn't reached its own draw loop yet — it's still decompressing its asset archives. This is **not a bug**: real forward progress through 46 distinct files was confirmed over a 15-minute window. It's a pure CPU-emulation-throughput problem. We profiled it carefully (two dead-end-then-real rounds, §80-82): a critical-section fast path was correctly ruled out (wrong bottleneck — 91.6% of *calls* but negligible *time*, and the only redirect mechanism on this backend would make it slower, not faster); a native zlib 1.1.4 high-level-emulation redirect for the game's actual decompression calls was found viable (confirmed the exact zlib version, entry points, and got a real profile: inflate is ~21-24% of loading wall-clock, peaking ~55-58% in bursts) but honestly bounded — only ~1.25-1.3x, since the remaining ~78% is diffuse across ordinary guest code (critical sections, syscalls, registry, engine logic), not concentrated anywhere else fixable.

**Why this matters for you specifically:** three rounds converged on the same conclusion — this is an aggregate CPU-emulation-throughput problem, not a fixable hot spot, and no amount of further per-function optimization on an interpreter backend can meaningfully close a "many hours" gap. A genuine JIT/dynarec compiler (what FEXCore is) characteristically yields order-of-magnitude throughput gains over interpretation for exactly this kind of sustained, CPU-bound workload (bulk decompression, tight critical-section loops, general engine code during asset loading) — the kind of gain no HLE redirect or hand-tuned fast path on this backend could match. **If FEXCore reaches a comparable point in MW2's startup (real D3D9 device created, past the DirectDraw/dxcore probe), it's plausible your backend gets through the loading phase in minutes rather than hours, and reaches MW2's actual draw loop first** — genuinely worth prioritizing getting FEXCore stable through MW2's early startup over chasing the same throughput problem on our side, since your architecture is the one actually suited to solving it.
