...
| M5 | MW2 integration (WoW64, SM3, 32-bit UMD) | **Not started, blocked on M3** | The WoW64/x86 UMD port itself is done (see the WoW64 row above), and its shader/texture/constant-register/texcoord features are now confirmed on the x86 path specifically (not just the fixed-function triangle) — the remaining blocker is real MW2 integration work: M3's remaining DDI coverage (cube/volume textures, formats beyond the M2 table, `stream_frequencies`/instancing). SM3.0 caps acceptance itself is now done (2026-07-06, see the M3 row above and "M3 coverage items" below) — real `d3d9.dll` accepts `vs_3_0`/`ps_3_0` and a genuine SM3.0-only shader construct renders correctly end to end on both x64 and x86/WoW64; the one residual, smaller uncertainty is that this doesn't guarantee every specific SM3.0 instruction a real, complex MW2 shader uses has been exercised. `D3DPOOL_MANAGED` is now fixed on both x64 and x86/WoW64 (see below) via a permanent runtime memory patch, not more DDI coverage — and since the 32-bit `syswow64/d3d9.dll` branch real MW2 (a 32-bit game) actually uses is now RE'd and wired too, MANAGED-pool asset loading is no longer a standing x86-only risk for MW2 integration. |
...
- [x] **Multi-stream vertex sources** — done. Real per-stream `SetStreamSource` byte offsets
  ...
  this work**: `stream_frequencies` (instancing) — see the still-open bullet below (`DrawPrimitiveUP`/
...
- [x] **SM3.0 caps — done (2026-07-06, commits `1b940580`/`d82434ff`/`c468e80d`).** The old limitation:
  ...
  **Residual uncertainty, stated honestly**: this closes SM3.0 CAPS acceptance and proves one genuine
  ...
  constructs (e.g. vertex texture fetch, `texldl`, additional interpolator behavior) this minimal test
  didn't exercise could still surface their own gaps once real MW2 shaders are actually run. Caps
  acceptance plus one working discriminator is real, major progress toward MW2 — it is not a claim of
  complete SM3.0 instruction-set coverage.
...
can proceed in roughly the order listed above. Cube/volume textures were believed, going into
