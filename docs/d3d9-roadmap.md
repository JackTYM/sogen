...
- [x] **Partial-buffer `Lock()` — fixed on x64 (2026-07-04, Task 6); now also fixed on x86/WoW64
  (2026-07-06, commit `ccd65a5d`).** `D3DDDIARG_LOCK`'s `OffsetToLock`/`SizeToLock` have no single,
  ...
  first chunk instead. **x86/WoW64 closed the same way (2026-07-06)**: a gated live-RE pass (two
  distinctive marker values, `0x4321`/`0x8642`, both landing at exactly byte offset 8 of the x86
  `D3DDDIARG_LOCK`, cross-checked against a `Lock(offset=0)` baseline reading 0 there) confirmed x86's
  driver-routed `OffsetToLock` sits at offset 8 (vs. offset 80 on x64 — different struct layout, same
  field, same fix). `umd_Lock`'s `#ifdef _WIN64`/`#else` split collapsed to one unconditional read for
  both architectures. `d3d9_partial_lock_test.cpp` cross-compiled to i686 and passes the identical
  discriminator against the real 32-bit `d3d9.dll`: chunk0's DISCARD-filled pattern survives unmodified
  after chunk1/chunk2's later NOOVERWRITE appends. Full trace: `HANDOFF_MACBOOK.md` §16.1, §21, §48.
...
- [x] **Cube/volume textures — done (2026-07-06, commits `713d2897`/`e3e26e64`/`05b31b49`/`39c8728a`/
  `fab1bcaa`/`d5d1a366`/`e783b93f`/`576b9480`).** A first investigation (same day) believed real
  `d3d9.dll` rejects `CreateCubeTexture`/`CreateVolumeTexture` with `D3DERR_INVALIDCALL` because of an
  opaque, runtime-side internal capability table inside `CEnum::CheckDeviceFormat` that no UMD-side
  `g_formats` edit could reach — a genuine wall, the same *kind* of problem `D3DPOOL_MANAGED` needed a
  runtime binary patch to clear. **A second, more careful gated re-investigation found that conclusion
  was wrong.** The prior patch attempt set FORMATOP bits `0x4`(cube)/`0x8000`/`0x10000`(volume) — but
  those tests live inside a code block gated behind `D3DUSAGE_AUTOGENMIPMAP` (`0x400`) that never
  executes for a plain create (`usage=0`), so the attempt never touched the real gate at all. Fresh
  disassembly of the actual per-format capability match loop (`CEnum::CheckDeviceFormat`,
  `0x18002F7F2`-`0x18002F824`) showed it reads a format's op-word DIRECTLY from the UMD's `GETFORMATDATA`
  DDI response — the exact same mechanism already proven UMD-controllable by the `A16B16G16R16F`/
  `FMT_OP_VERTEXTEXTURE` work above — and ANDs it against the standard `D3DFORMAT_OP_CUBETEXTURE`
  (`0x4`)/`D3DFORMAT_OP_VOLUMETEXTURE` (`0x2`) bits for a plain cube/volume create. `g_formats` already
  defined both constants but applied them to zero rows — that was the entire gate. No `d3d9.dll` binary
  patch needed; a plain UMD data fix, arch-agnostic (`g_formats` has no `_WIN64` split).
  **Live-confirmed during the gated pass**: `D3DDDIARG_CREATERESOURCE::Flags` (offset 56 x64/48 x86)
  carries an unambiguous CubeMap (`0x20000`)/Volume (`0x40000`) classifier bit, independent of the
  RenderTarget/Dynamic bits already handled; `SurfCount=6` for a cube create (one surface per face),
  `SurfCount=1` with `pSurfList[0].Depth`=real depth for volume (already flowed to `req.depth`); and the
  previously *inferred* `D3DDDIARG_LOCK::SubResourceIndex` formulas are now live-confirmed exact — cube
  `face*MipLevels+level` (6 face locks → indices 0-5), volume `level` (one full 3D block per `LockBox`
  call, no per-slice sub-locking).
  **Implementation**: UMD `g_formats` gained `FMT_OP_CUBETEXTURE` on `A8R8G8B8`/`X8R8G8B8`/DXT1/3/5 and
  `FMT_OP_VOLUMETEXTURE` on `A8R8G8B8`/`X8R8G8B8` only (compressed volume deliberately excluded — real
  D3D9 apps rarely use it); a new `resource_flags_to_kind` classifies each create from the Flags bits.
  Host-side, one shared `texture_subresource_layout(kind, ...)` helper (consumed identically by both the
  `create_resource` backing-store sizing site and the `ensure_texture_uploaded` staging-upload site, so
  the two can never disagree about which subresource index holds which level/face) drives real Vulkan
  images: cube → `VK_IMAGE_TYPE_2D`, `array_layers=6`, `VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT`; volume →
  `VK_IMAGE_TYPE_3D`, real depth. A second shared helper, `sampled_view_shape_for_kind`, drives both the
  PS and VS sampler-view-creation sites identically (cube → `VIEW_TYPE_CUBE`/6 layers, volume →
  `VIEW_TYPE_3D`/1 layer). `d3d9_shader_translator.cpp` needed ZERO changes — vkd3d-shader derives
  sampler dimension from the shader bytecode's own `dcl_cube`/`dcl_volume` tokens for SM2.0+, confirmed a
  non-blocker; the descriptor-write code and `pipeline_cache_key` likewise needed no changes (sampler
  dimension is shader-derived, not pipeline-construction state).
  **Proof**: a creation-only negative/positive discriminator (`d3d9_cube_volume_test.cpp`) plus two real
  SAMPLING discriminators — `d3d9_cube_test.cpp` fills each of a cube texture's 6 faces a distinct color
  and samples all 6 via `texCUBE` at each face's canonical center direction (fed through a PS shader
  constant), asserting each sub-pass against its OWN face's color; `d3d9_volume_test.cpp` fills a
  4-slice volume texture and samples all 4 via `tex3D` at each slice's center `w` coordinate, same
  per-sub-pass assertion discipline. Both are genuine discriminators (a wrong flattened index or a
  collapsed depth extent would make multiple sub-passes read back the SAME wrong color instead of N
  genuinely distinct correct ones) — proven byte-exact on both x64 and x86/WoW64, the x86 port needing
...
- [x] **WoW64/x86 shader path** — done, see the WoW64 row in the milestone table above. Typed
  ...
  found zero new x86-only bugs (a contrast worth noting given how many earlier ports did). Partial-buffer
  Lock support is now fixed on x86 too (2026-07-06, see "Known bugs & limitations" above). Of the 5
  low-confidence DDI slot arities, 2 (`pfnFlush1`, `pfnCheckCounterInfo`) are now RE-confirmed correct
  and a third (`pfnCheckCounter`) is a suspected-but-not-solidly-confirmed mismatch — see "Known bugs &
  limitations" above for the full account, including the finding that none of these 4 slots appear
  reachable from any ordinary D3D9 app code path. **Still open:** `pfnSetMarker`/`pfnSetMarkerMode`'s
  real arities remain genuinely unverified.
...
partial-buffer Lock is now genuinely fixed with real code, on both x64 (2026-07-04) and x86/WoW64
(2026-07-06, its own live-RE'd `OffsetToLock` offset); and `D3DPOOL_MANAGED`
...
M3's one remaining DDI-coverage item, cube/volume textures, is now also done (2026-07-06) — **M3's entire
checklist is closed.** A first same-day investigation believed real `d3d9.dll` rejects
`CreateCubeTexture`/`CreateVolumeTexture` with `D3DERR_INVALIDCALL` because of an opaque internal
capability cache inside `CEnum::CheckDeviceFormat` that no UMD-side `g_formats` edit could reach — a
genuinely deeper wall than the "just confirm a classifier bit" framing a prior planning pass believed
sufficient. A second, more careful gated re-investigation found that conclusion was wrong: the first
patch attempt tested FORMATOP bits inside a code block gated behind `D3DUSAGE_AUTOGENMIPMAP` that never
executes for a plain create, so it never reached the real gate at all. The actual per-format capability
match loop reads the UMD's op-word directly — the same mechanism already proven UMD-controllable by the
`A16B16G16R16F`/`FMT_OP_VERTEXTEXTURE` work — so this was a plain `g_formats` data fix, not a binary
patch. See the cube/volume bullet above for the full account, including the confirmed non-blocker (the
shader translator needs no changes for cube/volume samplers under SM2.0+) and the live-confirmed
`D3DDDIARG_CREATERESOURCE::Flags` classifier bits/`SubResourceIndex` formulas that made the implementation
possible. `D3DPOOL_MANAGED` is now fixed on both x64 and
...
`d3d9_managed_texture_test-x86.exe`), so this is no longer a budgeted risk for MW2 integration. x86
partial-buffer Lock support is also now done (2026-07-06, commit `ccd65a5d`) — the same kind of live-RE
pass found x86's driver-routed `OffsetToLock` at offset 8, and `d3d9_partial_lock_test.cpp` passes
identically on both architectures — closing the last standing gap this section used to flag for future
MW2-integration work.
