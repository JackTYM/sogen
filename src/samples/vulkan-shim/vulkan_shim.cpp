...
namespace
{
    ...
    // requests locally with zero IOCTLs until the surplus is exhausted. count>1, multi-layout, and
    // variable-descriptor-count requests fall back to the direct passthrough path unchanged, since
    // the readahead fetch below has no way to size a variable-length binding per request. Resetting
    // or destroying the owning pool invalidates every descriptor set ever allocated from it
    // host-side, including any surplus never handed to DXVK, so both invalidate this cache for that
    // pool. Touched by every DXVK thread (same reasoning as g_pending_descriptor_updates_mutex), so
    // it needs the same lock.
...
extern "C"
{
                                                                                  ...
                                                                                  VkDescriptorSet* pDescriptorSets)
    {
        ...
        if (count == 1 && variable_descriptor_count_count == 0)
        {
