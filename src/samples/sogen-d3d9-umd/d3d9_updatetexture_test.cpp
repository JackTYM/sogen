// IDirect3DDevice9::UpdateTexture regression test: a D3DPOOL_SYSTEMMEM staging texture is filled
// through LockRect/UnlockRect, marked with AddDirtyRect, pushed into a D3DPOOL_DEFAULT texture with
// UpdateTexture, and the DEFAULT texture is then sampled by a textured quad.
//
// This is the exact "streaming video texture" pattern Modern Warfare 2's Bink playback uses for its
// loading-screen YUV planes (recovered from iw4sp.exe's own disassembly: AddDirtyRect per plane,
// then one UpdateTexture per plane into the sampled DEFAULT-pool copy), which rendered from an
// all-zero destination under this UMD while the source was being written every frame.

#include <windows.h>
#include <d3d9.h>
#include <d3dcompiler.h>
#include <cstdio>
#include <cstring>
#include <cstdlib>

namespace
{
    // Same UV-through-COLOR0 workaround the sibling texture tests use -- this test targets
    // UpdateTexture, not TEXCOORD0 interpolation.
    const char* const k_vertex_shader_hlsl = R"(
struct VSInput { float3 pos : POSITION; float4 color : COLOR0; };
struct VSOutput { float4 pos : POSITION; float4 color : COLOR0; };
VSOutput main(VSInput input)
{
    VSOutput output;
    output.pos = float4(input.pos, 1.0);
    output.color = input.color;
    return output;
}
)";

    const char* const k_pixel_shader_hlsl = R"(
sampler2D s0 : register(s0);
struct PSInput { float4 pos : POSITION; float4 color : COLOR0; };
float4 main(PSInput input) : COLOR0
{
    return tex2D(s0, input.color.rg);
}
)";

    struct Vertex
    {
        float x, y, z;
        DWORD color;
    };

    constexpr DWORD kFvf = D3DFVF_XYZ | D3DFVF_DIFFUSE;
    constexpr int kCanvasWidth = 640;
    constexpr int kCanvasHeight = 480;
    constexpr int kTextureWidth = 64;
    constexpr int kTextureHeight = 64;

    float to_ndc_x(const int screen_x)
    {
        return static_cast<float>(screen_x) / (kCanvasWidth / 2) - 1.0f;
    }

    float to_ndc_y(const int screen_y)
    {
        return 1.0f - static_cast<float>(screen_y) / (kCanvasHeight / 2);
    }

    bool channel_close(const unsigned char actual, const int expected, const int tolerance)
    {
        return std::abs(static_cast<int>(actual) - expected) <= tolerance;
    }
} // namespace

int main()
{
    setvbuf(stdout, nullptr, _IOFBF, 1 << 16);
    printf("[d3d9-updatetexture-test] start\n");

    WNDCLASSA wc{};
    wc.lpfnWndProc = DefWindowProcA;
    wc.hInstance = GetModuleHandleA(nullptr);
    wc.lpszClassName = "sogend3d9updatetexturetest";
    RegisterClassA(&wc);
    HWND hwnd = CreateWindowExA(0, wc.lpszClassName, "updatetexture-test", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0, 0, kCanvasWidth,
                                kCanvasHeight, nullptr, nullptr, wc.hInstance, nullptr);
    ShowWindow(hwnd, SW_SHOW);
    UpdateWindow(hwnd);

    IDirect3D9* d3d = Direct3DCreate9(D3D_SDK_VERSION);
    if (!d3d)
    {
        printf("[d3d9-updatetexture-test] FAIL: Direct3DCreate9 returned null\n");
        return 1;
    }

    D3DPRESENT_PARAMETERS pp{};
    pp.Windowed = TRUE;
    pp.SwapEffect = D3DSWAPEFFECT_COPY;
    pp.Flags = D3DPRESENTFLAG_LOCKABLE_BACKBUFFER;
    pp.BackBufferFormat = D3DFMT_X8R8G8B8;
    pp.BackBufferWidth = kCanvasWidth;
    pp.BackBufferHeight = kCanvasHeight;
    pp.hDeviceWindow = hwnd;

    IDirect3DDevice9* dev = nullptr;
    HRESULT hr = d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hwnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &pp, &dev);
    printf("[d3d9-updatetexture-test] CreateDevice hr=0x%08lx dev=%p\n", static_cast<unsigned long>(hr), static_cast<void*>(dev));
    if (FAILED(hr) || !dev)
    {
        d3d->Release();
        return 1;
    }

    ID3DBlob* vs_blob = nullptr;
    ID3DBlob* vs_errors = nullptr;
    HRESULT hvsc = D3DCompile(k_vertex_shader_hlsl, strlen(k_vertex_shader_hlsl), nullptr, nullptr, nullptr, "main", "vs_2_0", 0, 0,
                              &vs_blob, &vs_errors);
    printf("[d3d9-updatetexture-test] D3DCompile(vs) hr=0x%08lx\n", static_cast<unsigned long>(hvsc));
    if (FAILED(hvsc))
    {
        if (vs_errors)
        {
            printf("[d3d9-updatetexture-test] VS compile errors: %s\n", static_cast<const char*>(vs_errors->GetBufferPointer()));
            vs_errors->Release();
        }
        dev->Release();
        d3d->Release();
        return 1;
    }

    ID3DBlob* ps_blob = nullptr;
    ID3DBlob* ps_errors = nullptr;
    HRESULT hpsc = D3DCompile(k_pixel_shader_hlsl, strlen(k_pixel_shader_hlsl), nullptr, nullptr, nullptr, "main", "ps_2_0", 0, 0, &ps_blob,
                              &ps_errors);
    printf("[d3d9-updatetexture-test] D3DCompile(ps) hr=0x%08lx\n", static_cast<unsigned long>(hpsc));
    if (FAILED(hpsc))
    {
        if (ps_errors)
        {
            printf("[d3d9-updatetexture-test] PS compile errors: %s\n", static_cast<const char*>(ps_errors->GetBufferPointer()));
            ps_errors->Release();
        }
        vs_blob->Release();
        dev->Release();
        d3d->Release();
        return 1;
    }

    IDirect3DVertexShader9* vs = nullptr;
    HRESULT hcvs = dev->CreateVertexShader(static_cast<const DWORD*>(vs_blob->GetBufferPointer()), &vs);
    IDirect3DPixelShader9* ps = nullptr;
    HRESULT hcps = dev->CreatePixelShader(static_cast<const DWORD*>(ps_blob->GetBufferPointer()), &ps);
    vs_blob->Release();
    ps_blob->Release();
    printf("[d3d9-updatetexture-test] CreateVertexShader hr=0x%08lx CreatePixelShader hr=0x%08lx\n", static_cast<unsigned long>(hcvs),
           static_cast<unsigned long>(hcps));
    if (FAILED(hcvs) || FAILED(hcps) || !vs || !ps)
    {
        if (vs)
        {
            vs->Release();
        }
        if (ps)
        {
            ps->Release();
        }
        dev->Release();
        d3d->Release();
        return 1;
    }

    IDirect3DTexture9* staging = nullptr;
    HRESULT hcs = dev->CreateTexture(kTextureWidth, kTextureHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &staging, nullptr);
    IDirect3DTexture9* sampled = nullptr;
    HRESULT hcd = dev->CreateTexture(kTextureWidth, kTextureHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &sampled, nullptr);
    printf("[d3d9-updatetexture-test] CreateTexture(SYSTEMMEM) hr=0x%08lx tex=%p / CreateTexture(DEFAULT) hr=0x%08lx tex=%p\n",
           static_cast<unsigned long>(hcs), static_cast<void*>(staging), static_cast<unsigned long>(hcd), static_cast<void*>(sampled));
    if (FAILED(hcs) || FAILED(hcd) || !staging || !sampled)
    {
        printf("[d3d9-updatetexture-test] FAIL: texture creation failed\n");
        if (staging)
        {
            staging->Release();
        }
        if (sampled)
        {
            sampled->Release();
        }
        vs->Release();
        ps->Release();
        dev->Release();
        d3d->Release();
        return 1;
    }

    int failures = 0;
    {
        D3DLOCKED_RECT lr{};
        HRESULT htl = staging->LockRect(0, &lr, nullptr, 0);
        printf("[d3d9-updatetexture-test] staging LockRect hr=0x%08lx pBits=%p Pitch=%ld\n", static_cast<unsigned long>(htl), lr.pBits,
               static_cast<long>(lr.Pitch));
        if (FAILED(htl) || !lr.pBits)
        {
            printf("[d3d9-updatetexture-test] FAIL: staging LockRect failed\n");
            staging->Release();
            sampled->Release();
            vs->Release();
            ps->Release();
            dev->Release();
            d3d->Release();
            return 1;
        }
        auto* base = static_cast<unsigned char*>(lr.pBits);
        for (int y = 0; y < kTextureHeight; ++y)
        {
            auto* row = reinterpret_cast<DWORD*>(base + y * lr.Pitch);
            for (int x = 0; x < kTextureWidth; ++x)
            {
                row[x] = D3DCOLOR_ARGB(255, 255, 0, 255); // solid magenta
            }
        }
        staging->UnlockRect(0);
    }

    HRESULT hadr = staging->AddDirtyRect(nullptr);
    HRESULT hut = dev->UpdateTexture(staging, sampled);
    printf("[d3d9-updatetexture-test] AddDirtyRect hr=0x%08lx UpdateTexture hr=0x%08lx\n", static_cast<unsigned long>(hadr),
           static_cast<unsigned long>(hut));
    if (FAILED(hut))
    {
        printf("[d3d9-updatetexture-test] FAIL: UpdateTexture failed\n");
        ++failures;
    }

    IDirect3DSurface9* rt = nullptr;
    HRESULT hcrt = dev->CreateRenderTarget(kCanvasWidth, kCanvasHeight, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0, TRUE, &rt, nullptr);
    if (FAILED(hcrt) || !rt)
    {
        printf("[d3d9-updatetexture-test] FAIL: render target creation failed\n");
        staging->Release();
        sampled->Release();
        vs->Release();
        ps->Release();
        dev->Release();
        d3d->Release();
        return 1;
    }
    dev->SetRenderTarget(0, rt);

    constexpr int kQuadW = 200;
    constexpr int kQuadH = 160;
    constexpr int kLeft = 40, kTop = 40;
    const auto uv_color = [](const float u, const float v) {
        return D3DCOLOR_ARGB(255, static_cast<BYTE>(u * 255.0f + 0.5f), static_cast<BYTE>(v * 255.0f + 0.5f), 0);
    };
    Vertex verts[4];
    verts[0] = {to_ndc_x(kLeft), to_ndc_y(kTop), 0.5f, uv_color(0.0f, 0.0f)};
    verts[1] = {to_ndc_x(kLeft + kQuadW), to_ndc_y(kTop), 0.5f, uv_color(1.0f, 0.0f)};
    verts[2] = {to_ndc_x(kLeft + kQuadW), to_ndc_y(kTop + kQuadH), 0.5f, uv_color(1.0f, 1.0f)};
    verts[3] = {to_ndc_x(kLeft), to_ndc_y(kTop + kQuadH), 0.5f, uv_color(0.0f, 1.0f)};

    IDirect3DVertexBuffer9* vb = nullptr;
    dev->CreateVertexBuffer(4 * sizeof(Vertex), 0, kFvf, D3DPOOL_DEFAULT, &vb, nullptr);
    if (vb)
    {
        Vertex* v = nullptr;
        vb->Lock(0, 4 * sizeof(Vertex), reinterpret_cast<void**>(&v), 0);
        if (v)
        {
            std::memcpy(v, verts, sizeof(verts));
            vb->Unlock();
        }
    }

    constexpr WORD kQuadIndices[6] = {0, 1, 2, 0, 2, 3};
    IDirect3DIndexBuffer9* ib = nullptr;
    dev->CreateIndexBuffer(6 * sizeof(WORD), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &ib, nullptr);
    if (ib)
    {
        WORD* idx = nullptr;
        ib->Lock(0, 6 * sizeof(WORD), reinterpret_cast<void**>(&idx), 0);
        if (idx)
        {
            std::memcpy(idx, kQuadIndices, sizeof(kQuadIndices));
            ib->Unlock();
        }
    }

    if (!vb || !ib)
    {
        printf("[d3d9-updatetexture-test] FAIL: vertex/index buffer creation failed\n");
        if (vb)
        {
            vb->Release();
        }
        if (ib)
        {
            ib->Release();
        }
        rt->Release();
        staging->Release();
        sampled->Release();
        vs->Release();
        ps->Release();
        dev->Release();
        d3d->Release();
        return 1;
    }

    dev->SetFVF(kFvf);
    dev->SetStreamSource(0, vb, 0, sizeof(Vertex));
    dev->SetIndices(ib);
    dev->SetVertexShader(vs);
    dev->SetPixelShader(ps);
    dev->SetTexture(0, sampled);
    dev->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_POINT);
    dev->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_POINT);
    dev->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
    dev->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
    dev->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);

    dev->BeginScene();
    dev->Clear(0, nullptr, D3DCLEAR_TARGET, D3DCOLOR_XRGB(64, 128, 255), 1.0f, 0);
    HRESULT hd0 = dev->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, 4, 0, 2);
    printf("[d3d9-updatetexture-test] DrawIndexedPrimitive hr=0x%08lx\n", static_cast<unsigned long>(hd0));
    dev->EndScene();

    D3DLOCKED_RECT lr{};
    HRESULT hlr = rt->LockRect(&lr, nullptr, D3DLOCK_READONLY);
    if (SUCCEEDED(hlr) && lr.pBits)
    {
        const auto* base = static_cast<const unsigned char*>(lr.pBits);
        const unsigned char* p = base + 120 * lr.Pitch + 140 * 4;
        printf("[d3d9-updatetexture-test] sampled pixel(140,120)=B=%02X G=%02X R=%02X (expected MAGENTA B=FF G=00 R=FF)\n", p[0], p[1],
               p[2]);
        if (!channel_close(p[0], 255, 2) || !channel_close(p[1], 0, 2) || !channel_close(p[2], 255, 2))
        {
            printf("[d3d9-updatetexture-test] FAIL: the UpdateTexture destination did not receive the staging texture's pixels\n");
            ++failures;
        }
        else
        {
            printf("[d3d9-updatetexture-test] PASS: UpdateTexture carried the SYSTEMMEM staging pixels into the sampled texture\n");
        }
        rt->UnlockRect();
    }
    else
    {
        printf("[d3d9-updatetexture-test] FAIL: render-target LockRect hr=0x%08lx\n", static_cast<unsigned long>(hlr));
        ++failures;
    }

    rt->Release();
    staging->Release();
    sampled->Release();
    vb->Release();
    ib->Release();
    vs->Release();
    ps->Release();
    dev->Release();
    d3d->Release();

    printf("[d3d9-updatetexture-test] %s\n", failures == 0 ? "ALL CHECKS PASSED" : "FAILED");
    return failures == 0 ? 0 : 1;
}
