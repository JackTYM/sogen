...
// (mach_vm_allocate/mach_vm_remap/mach_vm_region/mach_vm_read_overwrite) are still real, linkable
// Mach traps on iOS - confirmed present in the iOS/iOS-Simulator SDKs' libSystem.tbd export list -
// only the convenience header is withheld. Declare just those routines ourselves, with the same
// signatures the withheld header would have provided (mach/mach_vm.h on the macOS SDK).
