...
namespace sogen::fex
{
    ...
    namespace
    {
        ...
        // DEBUG AID (kept intentionally, cheap): a fixed-size ring buffer of recent fault_signal_handler invocations, recording
        // register state right at fault entry (before this handler can redirect PC/registers for a
        // retry) plus whether it was ultimately resolved. Investigating a deterministic crash whose
        // leading hypothesis is that some *earlier*, distinct fault (most likely the JIT write-protect
        // race above) resolves in a way that leaves general-purpose registers subtly wrong before a
        // *later*, unrelated fault is observed - this buffer lets the terminal crash print show the
        // handful of fault events immediately preceding the actual crash to test that directly.
        // Async-signal-safe: fixed array, no allocation, no locking (benign torn reads/writes under
        // genuine concurrent signals are acceptable for a diagnostic aid, unlike the correctness-
        // critical retry-count mechanism above).
        struct fault_event
        {
            uint64_t seq = 0;
        ...
        constexpr size_t fault_event_ring_size = 64;
        fault_event g_fault_event_ring[fault_event_ring_size];
        std::atomic<uint64_t> g_fault_event_seq{0};

        // Records a fault at entry (before handle_fault_signal can mutate uc_mcontext for a retry/
        // redirect) and returns the slot index so the caller can update `handled` once known.
        size_t record_fault_event_entry(int sig, siginfo_t* info, const ucontext_t* uctx)
        {
            const uint64_t seq = g_fault_event_seq.fetch_add(1, std::memory_order_relaxed);
            const size_t index = seq % fault_event_ring_size;
            auto& event = g_fault_event_ring[index];
            event.seq = seq;
            event.fault_addr = reinterpret_cast<uint64_t>(info->si_addr);
            event.pc = arm_thread_state64_get_pc(uctx->uc_mcontext->__ss);
            event.x0 = uctx->uc_mcontext->__ss.__x[0];
            event.x28 = uctx->uc_mcontext->__ss.__x[28];
            // Reading *pc directly (not decoding) captures the exact instruction encoding a decode
            // failure couldn't otherwise reveal - but only safe when pc is genuinely executable code.
            // An instruction-fetch fault (si_addr == pc, e.g. a real wild/garbage branch - a
            // pre-existing bug this ring buffer exists to help find, not something to assume away)
            // means pc itself is NOT valid, readable memory - dereferencing it here would fault a
            // second time recording the first fault, corrupting the very diagnostic meant to observe
            // it. Skip capturing insn in that case; every other fault kind (data access, self-
            // modifying-write race) has pc pointing at real, executing code.
            event.insn = (event.fault_addr == event.pc) ? 0 : *reinterpret_cast<const uint32_t*>(event.pc);
            event.sig = sig;
            event.si_code = info->si_code;
            event.handled = false;
            return index;
        }
        ...
        // stores; a plain-store false fault falls through to a crash instead of being emulated - a
        // known, narrower-than-intended gap, not a silent correctness issue (guard/unmapped violations
    ...
    class fex_x86_64_emulator final : public x86_64_emulator
    {
      ...
      public:
        ...
        // free-function fault_signal_handler's terminal crash print compare x28 (STATE) against what
        // it should be. thread_/CurrentFrame are otherwise private.
        uint64_t debug_expected_state_pointer() const
        {
            return this->active_thread_ != nullptr ? reinterpret_cast<uint64_t>(this->active_thread_->CurrentFrame) : 0;
        }

        bool debug_is_address_in_code_buffer(uint64_t address) const
        {
            return this->active_context_ && this->active_context_->IsAddressInCodeBuffer(this->active_thread_, address);
        }

        uint64_t debug_current_exit_function_link_ptr() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->Pointers.ExitFunctionLink : 0;
        }

        uint64_t debug_last_exit_linker_state() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->DebugLastExitLinkerState : 0;
        }

        uint64_t debug_last_exit_linker_state_after_spill() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->DebugLastExitLinkerStateAfterSpill : 0;
        }

        uint64_t debug_x0_after_mov() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->DebugX0AfterMov : 0;
        }

        uint64_t debug_x0_before_call() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->DebugX0BeforeCall : 0;
        }

        uint64_t debug_x2_before_call() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->DebugX2BeforeCall : 0;
        }

        // TEMP diagnostic: the guest-visible x86 RIP at crash time, read only from the crash handler
        // (which runs post-facto, after the fault already happened) so it costs nothing on the hot
        // path unlike every other diagnostic tried for this investigation, all of which perturbed
        // timing enough to turn this deterministic crash into an infinite hang instead.
        uint64_t debug_guest_rip() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->State.rip : 0;
        }

        // The GuestRIP of the last block ContextImpl::CompileBlock successfully compiled (see
        // DebugLastCompiledGuestRip's doc comment, deps/FEX's CoreState.h) - the closest thing to
        // "where execution was coming from" for a crash whose own faulting RIP is already
        // overwritten with a bad target by the time any handler observes it (e.g. a call/jmp through
        // a corrupt or unpopulated function pointer, where the target itself carries no information
        // about the call site). Same zero-cost, crash-handler-only reasoning as debug_guest_rip.
        uint64_t debug_last_compiled_guest_rip() const
        {
            return this->active_thread_ != nullptr ? this->active_thread_->CurrentFrame->DebugLastCompiledGuestRip : 0;
        }
    ...
    namespace
    {
        ...
        {
            auto* uctx = static_cast<ucontext_t*>(raw_ucontext);
            const size_t fault_event_index = record_fault_event_entry(sig, info, uctx);

            const bool handled = g_active_emulator != nullptr && g_active_emulator->handle_fault_signal(sig, info, raw_ucontext);
            g_fault_event_ring[fault_event_index].handled = handled;
            if (handled)
            {
                return;
            }
            ...
            // DEBUG AID (kept intentionally, cheap): raw hex dump of uc_mcontext and the mcontext64 struct's own header fields,
            // to rule out a struct-definition/versioning mismatch between what we compiled against
            // and what the kernel actually populated (e.g. an unexpected exception-state header
            // shifting where __ss actually starts).
            {
                char hex_buf[700];
                int hex_len = snprintf(hex_buf, sizeof(hex_buf),
                                       "[FEX backend] raw mcontext ptr=%p sizeof(*mcontext)=%zu es_offset=%zu ss_offset=%zu ns_offset=%zu "
                                       "bytes_at_mcontext:",
                                       static_cast<void*>(uctx->uc_mcontext), sizeof(*uctx->uc_mcontext),
                                       offsetof(std::remove_pointer_t<decltype(uctx->uc_mcontext)>, __es),
                                       offsetof(std::remove_pointer_t<decltype(uctx->uc_mcontext)>, __ss),
                                       offsetof(std::remove_pointer_t<decltype(uctx->uc_mcontext)>, __ns));
                const auto* raw_bytes = reinterpret_cast<const unsigned char*>(uctx->uc_mcontext);
                for (size_t i = 0; i < 64 && hex_len > 0 && static_cast<size_t>(hex_len) < sizeof(hex_buf) - 4; ++i)
                {
                    hex_len += snprintf(hex_buf + hex_len, sizeof(hex_buf) - static_cast<size_t>(hex_len), " %02x", raw_bytes[i]);
                }
                if (hex_len > 0)
                {
                    const auto hex_write_len =
                        static_cast<size_t>(hex_len) < sizeof(hex_buf) ? static_cast<size_t>(hex_len) : sizeof(hex_buf);
                    ::write(STDERR_FILENO, hex_buf, hex_write_len);
                    ::write(STDERR_FILENO, "\n", 1);
                }
            }

            // DEBUG AID (kept intentionally, cheap): dump the last few fault_event ring-buffer entries (see its doc comment) -
            // tests whether an earlier, distinct fault occurred immediately before this crash and how
            // it was resolved, which would point at that resolution path leaving registers wrong
            // rather than anything in the crashing call itself.
            {
                const uint64_t total_seq = g_fault_event_seq.load(std::memory_order_relaxed);
                const uint64_t dump_count = total_seq < 10 ? total_seq : 10;
                for (uint64_t i = 0; i < dump_count; ++i)
                {
                    const uint64_t seq = total_seq - 1 - i;
                    const auto& event = g_fault_event_ring[seq % fault_event_ring_size];
                    if (event.seq != seq)
                    {
                        break;
                    }
                    char ring_buf[240];
                    const int ring_len = snprintf(
                        ring_buf, sizeof(ring_buf),
                        "[FEX backend] fault_event seq=%llu sig=%d si_code=%d fault_addr=0x%llx pc=0x%llx "
                        "insn=0x%08x x0=0x%llx x28=0x%llx handled=%d\n",
                        static_cast<unsigned long long>(event.seq), event.sig, event.si_code,
                        static_cast<unsigned long long>(event.fault_addr), static_cast<unsigned long long>(event.pc), event.insn,
                        static_cast<unsigned long long>(event.x0), static_cast<unsigned long long>(event.x28), event.handled ? 1 : 0);
                    if (ring_len > 0)
                    {
                        const auto ring_write_len =
                            static_cast<size_t>(ring_len) < sizeof(ring_buf) ? static_cast<size_t>(ring_len) : sizeof(ring_buf);
                        ::write(STDERR_FILENO, ring_buf, ring_write_len);
                    }
                }
            }

            // TEMP diagnostic for the mystery memory-corruption investigation: this path is already
            // fatal (SIG_DFL + raise below), so calling dladdr here (not async-signal-safe in general)
            // is an acceptable one-shot risk - it symbolizes the crash pc/lr against whatever loaded
            // image they fall in (independently - pc can fail to resolve, e.g. pc==0, while lr still
            // usefully resolves), and compares x28 (STATE) against what it should be, which no other
            // diagnostic so far has captured.
            {
                const auto& gprs = uctx->uc_mcontext->__ss;
                const uint64_t lr = gprs.__lr;
                const uint64_t x28 = gprs.__x[28];
                const uint64_t expected_x28 = g_active_emulator != nullptr ? g_active_emulator->debug_expected_state_pointer() : 0;

                Dl_info pc_info{};
                const bool pc_resolved = ::dladdr(reinterpret_cast<void*>(pc), &pc_info) != 0;
                Dl_info lr_info{};
                const bool lr_resolved = lr != 0 && ::dladdr(reinterpret_cast<void*>(lr), &lr_info) != 0;
                Dl_info x28_info{};
                const bool x28_resolved = x28 != 0 && ::dladdr(reinterpret_cast<void*>(x28), &x28_info) != 0;
                const bool x28_in_code_buffer = g_active_emulator != nullptr && g_active_emulator->debug_is_address_in_code_buffer(x28);

                char sym_buf[512];
                const int sym_len =
                    snprintf(sym_buf, sizeof(sym_buf),
                             "[FEX backend] pc sym=%s+0x%llx img=%s | lr=0x%llx sym=%s+0x%llx img=%s | x28=0x%llx expected_x28=0x%llx "
                             "x28_img=%s x28_in_codebuf=%d | sp=0x%llx\n",
                             pc_resolved && pc_info.dli_sname ? pc_info.dli_sname : "?",
                             pc_resolved ? static_cast<unsigned long long>(pc - reinterpret_cast<uint64_t>(pc_info.dli_saddr)) : 0ULL,
                             pc_resolved && pc_info.dli_fname ? pc_info.dli_fname : "?", static_cast<unsigned long long>(lr),
                             lr_resolved && lr_info.dli_sname ? lr_info.dli_sname : "?",
                             lr_resolved ? static_cast<unsigned long long>(lr - reinterpret_cast<uint64_t>(lr_info.dli_saddr)) : 0ULL,
                             lr_resolved && lr_info.dli_fname ? lr_info.dli_fname : "?", static_cast<unsigned long long>(x28),
                             static_cast<unsigned long long>(expected_x28), x28_resolved && x28_info.dli_fname ? x28_info.dli_fname : "?",
                             x28_in_code_buffer ? 1 : 0, static_cast<unsigned long long>(gprs.__sp));
                if (sym_len > 0)
                {
                    const auto sym_write_len =
                        static_cast<size_t>(sym_len) < sizeof(sym_buf) ? static_cast<size_t>(sym_len) : sizeof(sym_buf);
                    ::write(STDERR_FILENO, sym_buf, sym_write_len);
                }

                // TEMP diagnostic: dump every GPR - checking whether the wild crash value (which has
                // been observed to repeat identically across independent ASLR-randomized runs, ruling
                // out ordinary heap garbage) also shows up in other registers, which would indicate a
                // constant-table load landing in the wrong register rather than a computed pointer.
                char gpr_buf[896];
                int gpr_len = snprintf(gpr_buf, sizeof(gpr_buf), "[FEX backend] gprs:");
                for (int i = 0; i < 29 && gpr_len > 0 && static_cast<size_t>(gpr_len) < sizeof(gpr_buf); ++i)
                {
                    gpr_len += snprintf(gpr_buf + gpr_len, sizeof(gpr_buf) - static_cast<size_t>(gpr_len), " x%d=0x%llx", i,
                                        static_cast<unsigned long long>(gprs.__x[i]));
                }
                if (gpr_len > 0)
                {
                    const auto gpr_write_len =
                        static_cast<size_t>(gpr_len) < sizeof(gpr_buf) ? static_cast<size_t>(gpr_len) : sizeof(gpr_buf);
                    ::write(STDERR_FILENO, gpr_buf, gpr_write_len);
                    ::write(STDERR_FILENO, "\n", 1);
                }

                // TEMP diagnostic: guest RIP + whether x0 (Frame, as passed to ExitFunctionLink) matches
                // the expected Thread->CurrentFrame pointer - read only here (post-crash, zero hot-path
                // cost) per debug_guest_rip's doc comment.
                if (g_active_emulator != nullptr)
                {
                    const uint64_t guest_rip = g_active_emulator->debug_guest_rip();
                    const uint64_t last_compiled_guest_rip = g_active_emulator->debug_last_compiled_guest_rip();
                    const uint64_t x0 = gprs.__x[0];
                    const uint64_t x1 = gprs.__x[1];
                    const bool x0_in_code_buffer = g_active_emulator->debug_is_address_in_code_buffer(x0);
                    const bool x1_in_code_buffer = g_active_emulator->debug_is_address_in_code_buffer(x1);
                    char rip_buf[280];
                    const int rip_len = snprintf(
                        rip_buf, sizeof(rip_buf),
                        "[FEX backend] guest_rip=0x%llx last_compiled_guest_rip=0x%llx x0=0x%llx expected_frame=0x%llx x0_matches=%d "
                        "x0_in_codebuf=%d x1=0x%llx x1_in_codebuf=%d\n",
                        static_cast<unsigned long long>(guest_rip), static_cast<unsigned long long>(last_compiled_guest_rip),
                        static_cast<unsigned long long>(x0), static_cast<unsigned long long>(expected_x28), x0 == expected_x28 ? 1 : 0,
                        x0_in_code_buffer ? 1 : 0, static_cast<unsigned long long>(x1), x1_in_code_buffer ? 1 : 0);
                    if (rip_len > 0)
                    {
                        const auto rip_write_len =
                            static_cast<size_t>(rip_len) < sizeof(rip_buf) ? static_cast<size_t>(rip_len) : sizeof(rip_buf);
                        ::write(STDERR_FILENO, rip_buf, rip_write_len);
                    }

                    const uint64_t current_ptr = g_active_emulator->debug_current_exit_function_link_ptr();
                    const uint64_t wrapper_addr = reinterpret_cast<uint64_t>(&exit_function_link_jit_write_wrapper);
                    char ptr_buf[160];
                    const int ptr_len =
                        snprintf(ptr_buf, sizeof(ptr_buf),
                                 "[FEX backend] CurrentFrame->Pointers.ExitFunctionLink=0x%llx wrapper=0x%llx still_wrapped=%d\n",
                                 static_cast<unsigned long long>(current_ptr), static_cast<unsigned long long>(wrapper_addr),
                                 current_ptr == wrapper_addr ? 1 : 0);
                    if (ptr_len > 0)
                    {
                        const auto ptr_write_len =
                            static_cast<size_t>(ptr_len) < sizeof(ptr_buf) ? static_cast<size_t>(ptr_len) : sizeof(ptr_buf);
                        ::write(STDERR_FILENO, ptr_buf, ptr_write_len);
                    }

                    const uint64_t last_trampoline_state = g_active_emulator->debug_last_exit_linker_state();
                    const uint64_t last_trampoline_state_after_spill = g_active_emulator->debug_last_exit_linker_state_after_spill();
                    const uint64_t x0_after_mov = g_active_emulator->debug_x0_after_mov();
                    const uint64_t x0_before_call = g_active_emulator->debug_x0_before_call();
                    const uint64_t x2_before_call = g_active_emulator->debug_x2_before_call();
                    const uint64_t wrapper_addr2 = reinterpret_cast<uint64_t>(&exit_function_link_jit_write_wrapper);
                    char trampoline_buf[320];
                    const int trampoline_len = snprintf(
                        trampoline_buf, sizeof(trampoline_buf),
                        "[FEX backend] state=0x%llx after_spill=0x%llx x0_after_mov=0x%llx x0_before_call=0x%llx x2_before_call=0x%llx "
                        "expected=0x%llx wrapper=0x%llx wrong_before_spill=%d wrong_after_spill=%d wrong_after_mov=%d "
                        "wrong_before_call=%d x2_matches_wrapper=%d\n",
                        static_cast<unsigned long long>(last_trampoline_state),
                        static_cast<unsigned long long>(last_trampoline_state_after_spill), static_cast<unsigned long long>(x0_after_mov),
                        static_cast<unsigned long long>(x0_before_call), static_cast<unsigned long long>(x2_before_call),
                        static_cast<unsigned long long>(expected_x28), static_cast<unsigned long long>(wrapper_addr2),
                        last_trampoline_state != expected_x28 ? 1 : 0, last_trampoline_state_after_spill != expected_x28 ? 1 : 0,
                        x0_after_mov != expected_x28 ? 1 : 0, x0_before_call != expected_x28 ? 1 : 0,
                        x2_before_call == wrapper_addr2 ? 1 : 0);
                    if (trampoline_len > 0)
                    {
                        const auto trampoline_write_len = static_cast<size_t>(trampoline_len) < sizeof(trampoline_buf)
                                                              ? static_cast<size_t>(trampoline_len)
                                                              : sizeof(trampoline_buf);
                        ::write(STDERR_FILENO, trampoline_buf, trampoline_write_len);
                    }
                }
            }
