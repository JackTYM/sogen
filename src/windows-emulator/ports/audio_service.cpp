    ...
    namespace
    {
        ...
        struct audio_service_port : rpc_port
        {
          ...
          private:
            ...
            // 1232-byte in-memory form is bloated by host pointers): a session GUID, nAvgBytesPerSec, an opaque
            // server cookie (the client just hands it back in opnums 8/9, which we ignore), and a few counts.
            // The shared render buffer is NOT in the payload - it rides in as an ALPC HANDLE message attribute.
            // We back it with a pagefile section the guest can map and attach its handle.
                                                 ...
                                                 std::vector<alpc_reply_handle>& reply_handles)
            {
                ...
                // validates during Initialize: buffer size, "DCPE" magic, the period (0x989680, in 100-ns units)
                // and the negotiated WAVEFORMATEXTENSIBLE. The rest of the buffer stays silent.
                ...
                // The op7 NDR references the render buffer as an sh_section handle (the _Struct_9 union arm).
                // rpcrt4!LRPC_SYSTEM_HANDLE_DATA::GetSystemHandle requires the handle's ObjectType to be a
                // single-bit mask whose table lookup (NdrSystemHandleResource) matches the expected resource;
                // bit 7 maps to Section (=6), so ObjectType must be 1<<7 = 0x80. A zero/wrong ObjectType makes
                // rpcrt4 __fastfail(FAST_FAIL_INVALID_ARG). Report SECTION access.
                ...
                // The 120-byte op7 [out] wire: a session GUID, nAvgBytesPerSec (+0x10), a
                // [system_handle(sh_file)] HANDLE (+0x18, null here), an i64 server cookie (+0x20), an i64
                // (+0x28), three default unions, then a union whose selector (+0x54 = 1) picks the sh_section
                // arm and whose handle index (+0x58 = 1) references the delivered render section.
                // Ndr64SystemHandleUnmarshall consumes 8 wire bytes per system handle and treats a 1-based
                // index (0 = null); Ndr64UnionUnmarshall reads the selectors.
