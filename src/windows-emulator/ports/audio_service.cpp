        ...
        struct audio_service_port : rpc_port
        {
                ...
                const auto log_result = [&](const NTSTATUS status) {
                    if (diag)
                    {
                        win_emu.log.warn("[audio-rpc-diag] #%llu procedure_id=%u -> status=0x%X\n",
                                         static_cast<unsigned long long>(call_index), procedure_id, static_cast<uint32_t>(status));
                    }
                    return status;
                };
          ...
          private:
                                          ...
                                          std::vector<alpc_reply_handle>& reply_handles)
            {
                ...
                // live audio service (tools/alpc_capture.py) for a 1-second buffer; the sample-area bounds (+0x04,
                // +0x168, +0x16C, +0x170) and the buffer duration (+0x150) are patched below to match this stream.
                //
                // CCrossProcessBaseClientEndpoint::Initialize (audioses.dll, this build) validates this header
                // field-by-field against the mapped section base; re-disassembled this round rather than trusted
                // from the capture, since the capture had regressed twice now (see commit 6ef62298 for the
                // original RE of the magic/selector fields):
                //   - "DCPE" magic at +0x0C0 (cmp dword ptr [esi+0C0h], 45504344h @ 0x100387B1) - a mismatch
                //     aborts with 0x887C0045 and the client tears the stream down immediately after
                //     CreateRemoteStream, without ever calling StartStream.
                //   - format-block size (=cbSize+0xC8) at +0x0C8, not +0x0C4 (cmp [esi+0C8h],eax @ 0x100389F6,
                //     eax = cbSize + 0xC8) - the capture had it 4 bytes earlier, landing on what the validator
                //     treats as reserved padding.
                //   - WAVEFORMATEXTENSIBLE at +0x17C, not +0x178 (lea ebx,[esi+17Ch] @ 0x100387CA, immediately
                //     after the magic check; cross-checked via cbSize at [esi+18Ch] == ebx+0x10 and wFormatTag at
                //     [esi+17Ch] == ebx+0) - same 4-byte-short regression as the format-block-size field above.
                //   - the whole +0x14C..+0x170 block sat 4 bytes early, the same regression class as the two fields
                //     above. Field roles re-derived from CCrossProcessBaseEndpoint::GetCurrentQueue (0x1007E927),
                //     CCrossProcessControl::Reset (0x1003CDE0) and
                //     CCrossProcessClientOutputEndpoint::GetOutputDataPointer (0x100342F0), all of which index the
                //     same header through the blob copy at header[0xC8]:
                //       +0x14C (blob+132) is the item count of the 64-byte SharedMessageQueueItem array at
                //         section + [+0x164], not a duration. Reset zero-fills that many items, so writing a
                //         duration there (10^7 for the 1 s capture, 2*10^5 for DirectSound's 20 ms buffer) makes
                //         the client walk megabytes off the end of the section - the C0000005 at audioses+0x3cf6c.
                //         sogen exposes no message queue, so the count is 0 and the array is never touched.
                //       +0x158 and +0x15C are the counts of two further such arrays whose base pointers Initialize
                //         only fills in for a secondary queue ([+0xB4] != -1); both must be 0 or Reset dereferences
                //         an uninitialized pointer. The capture's -1 at +0x15C was a 4-billion-item count.
                //       +0x168/+0x16C bound the sample area as byte offsets from the section base, and must
                //         satisfy [+0x168] < [+0x16C] <= [+0x170] (v11[90]>=v11[91] || v11[91]>v11[92] fails @
                //         0x10038A08-0x10038A1A). GetOutputDataPointer hands the client
                //         section + [+0x168] + position % ([+0x16C] - [+0x168]), so the lower bound is the sample
                //         area's own offset, not 0: with 0 there the client wrote its PCM over the control header
                //         and ran a ring 0x400 bytes longer than the one this service drains.
                std::array<uint8_t, 0x1c4> render_control_header = {
                    0x01, 0x00, 0x00, 0x00, 0x20, 0x66, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // version, buffer size
                    ...
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 0x0B4: size selector
                    0x44, 0x43, 0x50, 0x45, 0x00, 0x00, 0x00, 0x00, 0xde, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 0x0C0: "DCPE", 0x0C8:
                    , // 0x14C: queue item count
                    0x80, 0x96, 0x98, 0x00, 0x00, 0x00, 0x00, 0x00, // 0x150: buffer duration
                    0x00, 0x00, 0x00, 0x00,                         // 0x158: aux queue count
                    0x00, 0x00, 0x00, 0x00,                         // 0x15C: secondary queue item count
                    0xff, 0xff, 0xff, 0xff,                         //
                    0x80, 0x01, 0x00, 0x00,                         // 0x164: queue array offset
                    0x00, 0x04, 0x00, 0x00,                         // 0x168: sample-area lower bound
                    0x20, 0x66, 0x05, 0x00,                         // 0x16C: sample-area upper bound
                    0x20, 0x66, 0x05, 0x00,                         // 0x170: sample-area maximum bound
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 0x174: reserved
                    0xfe, 0xff, 0x02, 0x00, 0x44, 0xac, 0x00, 0x00, 0x20, 0x62, 0x05, 0x00, 0x08, 0x00, 0x20, 0x00, // 0x17C:
                    0x16, 0x00, 0x20, 0x00, 0x03, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x80, 0x00, 0x00, 0xaa,
                    0x00, 0x38, 0x9b, 0x71, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                ...
                patch32(0x168, k_render_data_offset);
                patch32(0x16C, buffer_extent);
                patch32(0x170, buffer_extent);
                std::memcpy(&render_control_header[0x150], &duration_hns, sizeof(duration_hns));

                if (getenv("EMULATOR_AUDIO_RPC_DIAG"))
                {
                    std::string hex;
                    for (size_t i = 0xB0; i < 0x1A0; ++i)
                    {
                        char buf[4];
                        snprintf(buf, sizeof(buf), "%02x ", render_control_header[i]);
                        hex += buf;
                    }
                    win_emu.log.warn("[audio-header-diag] size=%zu bytes[0xB0..0x1A0)=%s\n", render_control_header.size(), hex.c_str());

                    const auto field32 = [&](const size_t offset) {
                        uint32_t value = 0;
                        std::memcpy(&value, &render_control_header[offset], sizeof(value));
                        return value;
                    };
                    win_emu.log.warn("[queue-index-diag] queue_count(header[0x14C])=%u aux_count(header[0x158])=%u "
                                     "secondary_count(header[0x15C])=%u indices=%u,%u lower(header[0x168])=0x%X "
                                     "upper(header[0x16C])=0x%X max(header[0x170])=0x%X -> client_frames=%u\n",
                                     field32(0x14C), field32(0x158), field32(0x15C), field32(0x00), field32(0x04), field32(0x168),
                                     field32(0x16C), field32(0x170), (field32(0x16C) - field32(0x168)) / k_block_align);
                }
