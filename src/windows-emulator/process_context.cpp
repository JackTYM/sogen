...
namespace sogen
{
    ...
    namespace
    {
        ...
        // CPTABLEINFO32 (kernel_mapped.hpp) is the layout real 32-bit ntdll parses under WoW64: it
        // differs from CPTABLEINFO only in pointer width, handled below via each field's own type.
        template <typename CpTableInfo>
        void fill_identity_codepage_table(emulator_allocator& allocator, CpTableInfo& t)
        {
            ...
            t.DBCSCodePage = 0;
            t.MultiByteTable = static_cast<decltype(t.MultiByteTable)>(multi_byte_table_addr);
            t.WideCharTable = static_cast<decltype(t.WideCharTable)>(wide_char_table_addr);
            t.DBCSRanges = 0;
            t.DBCSOffsets = 0;
        }
        ...
        uint32_t make_identity_codepage_table32(emulator_allocator& allocator)
        {
            ...
            table.access([&](CPTABLEINFO32& t) { fill_identity_codepage_table(allocator, t); });
