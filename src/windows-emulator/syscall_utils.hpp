...
namespace sogen
{
    ...
    inline void debug_log_syscall_caller_stack(const syscall_context& c, const char* tag)
    {
        ...
        fprintf(stderr, "[%s] pid=%u tid=%u rip=0x%llx (%s+0x%llx) rsp=0x%llx\n", tag, c.proc.process_id, c.thread().id,
        ...
        for (uint64_t i = 0; i < 64; ++i)
        {
            ...
            fprintf(stderr, "  [rsp+0x%llx] = 0x%llx %s\n", static_cast<unsigned long long>(i * 8), static_cast<unsigned long long>(value),
                    mod_suffix);
        }
