...
namespace sogen
{
    ...
    namespace
    {
        // TEB64/TEB32's NlsCache field is a pointer real ntdll allocates and populates with per-thread
        // cached NLS/locale data; real ntdll's Unicode case-mapping helpers read a field at a fixed
        // offset within it unconditionally, crashing on a null NlsCache.
        //
        // The correct value - and the one used whenever available - is the guest address of
        // kernelbase.dll's real gNlsProcessLocalCache global (see process_context::
        // kernelbase_nls_process_local_cache), exactly matching what a freshly-created real Windows
        // thread's TEB.NlsCache points at before it ever does its own locale/NLS API work. This is
        // load-bearing, not just a null-deref fix: kernelbase.dll's BaseNlsThreadCleanup (called on
        // DLL_THREAD_DETACH) frees whatever TEB.NlsCache points at via RtlFreeHeap unless it still
        // equals &gNlsProcessLocalCache exactly, so any other non-null placeholder gets passed to
        // RtlFreeHeap on every thread's exit, corrupting the heap.
        //
        // This zeroed placeholder is kept only as a defensive fallback for the case kernelbase.dll
        // isn't mapped yet: a zeroed cache reads as "nothing cached yet", avoiding the null-deref, but
        // callers must never let a thread using it reach BaseNlsThreadCleanup.
