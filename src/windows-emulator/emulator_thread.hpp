...
namespace sogen
{
    ...
    class emulator_thread : public ref_counted_object
    {
      public:
        ...
        uint64_t stack_reserve_base{};            // Lowest address of the native stack's whole reserved VA range (== TEB.DeallocationStack)
        uint64_t stack_guard_page{};              // Current PAGE_GUARD boundary below the committed stack; 0 once fully consumed
        ...
        // Called when a guard-page fault lands exactly on this thread's stack_guard_page. Mirrors
        // real Windows on-demand stack growth: commits the faulted page, removes its guard, and
        // re-arms a new guard one page further down - unless the guaranteed reserve at the bottom
        // of the stack has already been reached, in which case it delivers a real
        // STATUS_STACK_OVERFLOW instead. Returns true if the fault was resolved by silent growth
        // (caller should restart the faulting instruction), false if an exception was dispatched
        ...
        void serialize_object(utils::buffer_serializer& buffer) const override
        {
            ...
            buffer.write(this->stack_reserve_base);
        ...
        void deserialize_object(utils::buffer_deserializer& buffer) override
        {
            ...
            buffer.read(this->stack_reserve_base);
      ...
      private:
        ...
        void release()
        {
            ...
            if (this->stack_base)
            {
                ...
                if (this->stack_reserve_base)
                {
                    this->memory_ptr->release_memory(this->stack_reserve_base, 0);
                    this->stack_reserve_base = 0;
