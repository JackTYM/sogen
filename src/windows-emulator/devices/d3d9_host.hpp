...
namespace sogen
{
    ...
    class d3d9_host
    {
      public:
        ...
        // out_block_bytes/out_block_texels describe the format's row addressing granularity, which is
        // what lets the guest place a rect-scoped lock's pointer -- see lock_response's own comment.
        int32_t lock(uint64_t resource, uint32_t subresource, uint32_t offset, uint32_t size, uint32_t flags, lock_view& out_view,
                     uint32_t& out_data_size, uint32_t& out_pitch, uint32_t& out_slice_pitch, uint32_t& out_block_bytes,
                     uint32_t& out_block_texels);
